 ...
 * 2. Paste this entire file into App.tsx (replace the default content)
...
function Button({ label, iconLeading, onPress, disabled, testID }: any) {
  return (
    <Pressable
    ...
    >
      {iconLeading?.show && iconLeading.name && (
...
function Checkbox({ state, onStateChange, testID }: any) {
  const isChecked = state === 'selected';
  return (
    <Pressable
      onPress={() => onStateChange?.(isChecked ? 'unselected' : 'selected')}
      testID={testID}
      style={[
        styles.checkbox,
        {
 ...
 */
function ListItem({ label, paragraph, leadingContent, trailingContent, divider, testID }: any) {
  return (
    <View>
      <View style={styles.listRow}>
        {leadingContent?.iconName && (
          <FontAwesome6
      ...
      {divider?.type === 'full' && <View style={styles.listDivider} />}
    </View>
  );
}
...
function OfflineStatusIcon({ variant, available }: { variant: 'light' | 'dark'; available: boolean }) {
...
function GridCard({
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.small }}>
          {isSkeleton ? (
            ...
            <Checkbox
              ...
              onStateChange={() => onSelectionChange?.(!isSelected, 'press')}
            />
          ) : (
            <>
              {onDismiss && (
  ...
  const renderBody = () => {
    if (isLoading) return <View style={styles.gcBody}><ActivityIndicator size="large" color={Colors.icon.primary} /></View>;
    if (isSkeleton) return <View style={styles.gcBody}><View style={styles.gcSkeletonBodyRect} /></View>;
    if (isError) return <View style={styles.gcBody}><FontAwesome6 name="circle-exclamation" size={Icons.xlarge} color={Colors.icon.error} /></View>;
    if (isPhoto) return <View style={styles.gcBody} />;
    if (body?.kind === 'icon' && body.iconName) {
      return (
  ...
  const renderFooter = () => {
    ...
    if (isSkeleton) return <View style={styles.gcFooter}><View style={styles.gcSkeletonFooterRect} /></View>;
    if (!name) return null;
...
const FILE_TYPE_ICON: any = {
  image: 'image',
  video: 'video',
  file: 'file-lines',
};

function Upload({ variant, files, multiple = true, accept = 'any', description, onUploadPress, onRemoveFile, disabled, testID }: any) {
  ...
  const renderGridTile = (file: any) => {
    const onDismiss = disabled ? undefined : () => onRemoveFile(file.id);
    if (file.status === 'loading') return <GridCard key={file.id} isLoading name={file.name} onDismiss={onDismiss} offlineStatus={file.offlineStatus} />;
    if (file.status === 'error') return <GridCard key={file.id} type="error" name={file.name} onDismiss={onDismiss} offlineStatus={file.offlineStatus} />;
    if (file.previewUri) return <GridCard key={file.id} type="photo" name={file.name} body={{ kind: 'image', uri: file.previewUri }} onDismiss={onDismiss} offlineStatus={file.offlineStatus} />;
    return <GridCard key={file.id} type="default" name={file.name} body={{ kind: 'icon', iconName: FILE_TYPE_ICON[file.type ?? 'file'] }} onDismiss={onDismiss} offlineStatus={file.offlineStatus} />;
  };
...
export default function App() {
  const [listFiles, setListFiles] = useState([
    { id: 'l1', name: 'Daily safety report.pdf', type: 'file' as const, description: '2.4 MB' },
    { id: 'l2', name: 'site-inspection.jpg', type: 'image' as const, status: 'loading' as const },
    { id: 'l3', name: 'training-video.mp4', type: 'video' as const, status: 'error' as const, description: 'Upload failed' },
  ]);
  const [gridFiles, setGridFiles] = useState([
    { id: 'g1', name: 'WHMIS-symbols.jpg', type: 'image' as const, previewUri: PHOTO_A, offlineStatus: 'available' as const },
    { id: 'g2', name: 'uploading.jpg', type: 'image' as const, status: 'loading' as const },
    { id: 'g3', name: 'failed.pdf', type: 'file' as const, status: 'error' as const },
    { id: 'g4', name: 'site-photo.jpg', type: 'image' as const, previewUri: PHOTO_B },
  ]);
  const [singleFile, setSingleFile] = useState<any[]>([
    { id: 's1', name: 'signature.jpg', type: 'image' as const, previewUri: PHOTO_A },
  ]);

  const removeFromList = (id: string) => setListFiles(fs => fs.filter(f => f.id !== id));
  const removeFromGrid = (id: string) => setGridFiles(fs => fs.filter(f => f.id !== id));
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.screenContent}>
      ...
      <Section title="Upload · single-file mode (no CTA when populated)">
        <Upload
          ...
          accept="image"
          onUploadPress={() => {}}
          onRemoveFile={(id: string) => setSingleFile([])}
        />
      </Section>
    </ScrollView>
  );
